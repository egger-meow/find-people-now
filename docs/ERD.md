...
54. **配對引擎健壯性與邊界修復（v1.45）**：
    - **全體人數硬條件保證**：N 方累積撮合時，累積維護所有候選的 `[greatest(min), least(max)]` 嚴格交集，不再只由 seed 收斂；成團前（`fn_create_activity_from_requests`）強制驗證實際成員數滿足所有參與 Request 的上下限（`v_total_joined >= v_req_min and (v_req_max is null or v_total_joined <= v_req_max)`），徹底杜絕超收或早產。
    - **非傳遞性條件的全量檢查**：運動等級相鄰容許與讀書科目比對皆為非傳遞性關係，候選需求必須與目前累積集合中的「每一個」需求皆相容，防止 Wildcard 或中間等級錯誤吸納互斥需求。
    - **封鎖檢查涵蓋所有成員**：下探至 `request_member`（`status = 'JOINED'`），含受邀朋友，雙向封鎖即阻擋撮合，保護全體成員。
    - **自足 Request 成團支援**：放寬 `fn_create_activity_from_requests` 支援 `array_length >= 1`，當單筆 Request 透過邀請朋友已達標自身 `min_participants`，引擎可直接為其建立 Activity 成團，解決設計備註 39 遺留之已知限制。
    - **並發與防重防呆**：候選若包含累積集合中已存在的同一使用者則阻擋合併；單一撮合並發衝突透過防禦性例外攔截，不中斷整輪排程。
