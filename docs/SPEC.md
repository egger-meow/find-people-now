...
> **v1.21 變更紀錄**（NYCU 在校生年限軟性提醒）：
> 1. 🟢 **僅 NYCU 適用（NTHU 這輪不做）**：僅當信箱網域為 `nycu.edu.tw` 時檢查；解析規則——取 `@` 符號前緊接著的最後兩位數字視為民國入學年（例如 `mg09` → 109 年、`cs15` → 115 年），不管前面代碼長度；信箱格式無法解析出這樣的數字時直接跳過此檢查，不視為錯誤。
> 2. 🟢 **依 `degree_level` 設定門檻**：`UNDERGRAD`=6 年、`MASTER`=4 年、`PHD`=7 年，「現在年份（民國）− 入學年 > 門檻」才視為超過（`>`，不是 `>=`）。
...
> **v1.33 變更紀錄**（`bio` 從選填改為註冊硬性門檻 + 擋 Dicebear 佔位頭像 + 個人檔案卡）：
> 1. 🔴 **`bio`（自我介紹）從選填改為註冊硬性門檻**：起因是配對成立後的成員名單要能點開更完整的「個人檔案卡」（照片＋自我介紹＋科系），但 bio 一直選填、大量使用者根本沒填，檔案卡形同虛設。改成跟 `avatar_url`/`contact_*` 同一層級的硬性門檻——`app_user.bio` 改 `NOT NULL DEFAULT ''`（既有 null 值 backfill 成 `''`），`complete_profile` 新增 `trim(p_bio) = ''` 檢查（`PROFILE_INCOMPLETE`/`BIO_REQUIRED`），與 `avatar_url` 既有的兩層防護（DB NOT NULL + RPC 非空字串檢查）完全對稱。註冊畫面（`complete_profile_screen.dart`）的自我介紹欄位文案加上引導提示（興趣、可以跟別人分享的經驗或技能），標籤從「自我介紹（選填）」改成「自我介紹」。
> 2. 🔴 **`complete_profile` 新增擋 Dicebear 自動產生佔位頭像的規則**：註冊時使用者可以完全不上傳照片、單靠 `complete_profile_screen.dart` 自動產生的 Dicebear 卡通頭像過關（`AVATAR_URL_REQUIRED` 只檢查「非空字串」，卡通頭像的網址一樣非空）——配對成立後彼此仍然認不出對方，違背這輪要解決的「比較好認人」目的。全 repo 唯一會產生「使用者沒有主動上傳」網址的來源就是 `api.dicebear.com`（`avatar_upload.dart` 的 `pickAndUploadAvatar` 完全沒有這類自動產生邏輯，也沒有任何人臉辨識能力，即沒有 ML 依賴可用），因此新增規則：`p_avatar_url` 若命中 `dicebear\.com` 直接擋下（`PROFILE_INCOMPLETE`/`PLACEHOLDER_AVATAR_NOT_ALLOWED`）。註冊畫面移除「隨機頭像」按鈕與自動預帶的隨機頭像，改成空白狀態直到使用者主動上傳，並在上傳按鈕旁加上引導文案（建議使用清楚露臉的個人照）。
...
> **v1.39 變更紀錄**（`get_campus_pulse` 修正成算人頭、不算 Request 筆數，第 13 節）：
> 1. 🔴 **Bug**：v1.26 上線後的 `get_campus_pulse` 用 `count(*)` 算 `match_request` 列數，語意上等於「算幾組 Request」而不是「算幾個人在等」。兩者在「每個 Request 都只有 owner 自己」的情況下數字相同，但一個 Request 可能已經透過邀請連結（第 6.1 節）帶了不只一位 `request_member`，這時列數會低估實際等待人數，且 UI 文案「N 組配對中」的「組」字更會讓使用者誤讀成「已經湊了 N 組」而不是「還缺人的 N 組各自都還沒湊滿」——使用者實測踩到：兩個各自獨立、都還缺 2 人的健身局被顯示成「2 組配對中」，望文生義以為快成局了。
> 2. 🟢 **修法**：`get_campus_pulse` 改成 `join request_member`（`status = 'JOINED'`）算實際人頭，不再用 `count(*)` 數 `match_request` 列。回傳欄位同時從 `request_count` 更名為 `person_count`，避免欄位名稱本身繼續帶著「算 Request 不是算人」的錯誤暗示。見 `20260806000000_campus_pulse_headcount.sql`。
> 3. 🟢 **UI 文案同步調整**：首頁氣氛指標 chip 從「{類型} · N 組配對中」改成「{類型} · N 人在等」，用詞跟後端語意一致。
...
> 2. 🟢 **新增 `rpc: get_campus_demands(school, campus)`**：`SECURITY DEFINER`，只查詢 `status = 'REQUESTING'` 且 `latest_start > now()` 的有效需求，依活動條件與人數範圍（含 `min_participants, max_participants`）分組聚合，回傳 `person_count`（實際等待中人頭數，排除已離開成員）與 `request_count`（組數）。時間維度篩選（`全部` / `現在` / `今天` / `明天`，支援跨日重疊）由前端客戶端實作過濾。完全匿名，絕不包含任何個資或請求識別碼，恪守盲配界限。
...
> 
> **v1.45 變更紀錄**（配對引擎健壯性與邊界修復：全體參與者硬條件保證、成員級雙向封鎖、延遲撮合時間防護、純邀請朋友自足成團與並發防呆，第 6、7、12、16 節）：
> 1. 🟢 **全體參與者硬條件保證**：
>    - **人數上下限**：N 方累積時，累積總人數必須同時滿足全體參與者的 `min_participants` 與 `max_participants`（`greatest(min) <= least(max)`）。成團前檢驗實際成員數達到最大下限且不超過最小上限，徹底防止低人數需求被灌爆超收或高人數需求被提早強行成團。
>    - **運動等級傳遞性**：候選需求必須與目前累積集合中的「每一個」需求相容（`not exists mr_acc ... not fn_sport_level_match`），防止傳遞性失效（例如種子 A 相容 B 與 C，但 B 與 C 跨級不相容）。
>    - **讀書目標一致性**：候選需求必須與目前累積集合中所有已指定的目標一致，防止 Wildcard 種子同時吸納互斥科目。
> 2. 🟢 **封鎖檢查全體成員覆蓋**：封鎖檢查（`user_block`）下探至 `request_member`（`status = 'JOINED'`），包含透過邀請連結加入的朋友；累積集合與候選集合之間任一成員存在雙向封鎖即阻擋撮合，杜絕受邀成員被撮合進封鎖對象的局。
> 3. 🟢 **延遲撮合與過期時間防護**：排除 `latest_start <= now()` 之過期需求；共同時間交集嚴格檢查 `greatest(earliest, now()) <= latest`，防止因延遲撮合而成立已在過去開始的活動。
> 4. 🟢 **純邀請朋友達標自足成團**：`fn_create_activity_from_requests` 放寬支援 `array_length >= 1`。當單筆 Request 透過邀請朋友湊滿自身 `min_participants`，引擎可直接為其建立 Activity 成團，解決多年自足多人團卡死在 `REQUESTING` 的已知架構限制。
> 5. 🟢 **並發防重與防呆**：候選需求若含有已在累積集合中的同一使用者，禁止合併，防止重複成團與主鍵衝突；`fn_create_activity_from_requests` 與 `commit_match` 加入 `MISMATCHED_ACTIVITY_TYPE_OR_CAMPUS` 防禦性檢查；成團呼叫處包裝例外攔截，單一撮合並發衝突不中斷排程。
...
- 🟢 **`bio`（自我介紹）欄位，註冊硬性門檻（v1.33 起）**：短文字，讓對方在配對成立後多一點資訊判斷「這是不是我想約的人」（例如「研究所碩一，平常喜歡打球、看電影」）。**不進入配對核心邏輯**（跟性別欄位一樣不可用於篩選），但從 v1.33 起跟頭像/聯絡方式同一層級，會卡住 Request 發起/加入前的註冊流程——原本選填時大量使用者根本沒填，配對成立後的個人檔案卡形同虛設，見 v1.33 變更紀錄
...
- 🟢 **NYCU 在校生年限軟性提醒（v1.21，僅 NYCU，NTHU 這輪不做）**：註冊時若能從信箱 local-part 最後兩位數字解析出民國入學年（例如 `mg09` → 109 年、`cs15` → 115 年；解析不出來直接跳過，不視為錯誤），且距今超過該學制門檻（`UNDERGRAD`=6 年／`MASTER`=4 年／`PHD`=7 年，`>` 不是 `>=`），`complete_profile` 送出前跳出一次性確認訊息，使用者確認後仍可繼續——這是機率性提醒，不是硬性擋門，見 v1.21 變更紀錄
...
13. ✅ **v1.45 已解決**：Matching Engine 升級為全體人數硬條件保障。N 方累積時嚴格維護全體交集 `[greatest(min), least(max)]`，每次候選併入即時縮緊；且成團前再次硬性檢驗總人數落在全體參與者各自的 `[min, max]` 範圍內，徹底杜絕小團被超收灌爆或大團被提早早產。
14. ✅ **v1.45 已解決**：`fn_create_activity_from_requests` 放寬支援 `array_length >= 1`，且 `fn_run_matching_engine` 支援單筆 Request 湊滿自身 `min_participants`（例如透過邀請連結湊滿 4 人羽球局）時直接自足成團轉為 `MATCHED`，徹底解開自足團無第三方可合併而永久卡死在 `REQUESTING` 的已知缺陷。
