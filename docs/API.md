...
| 1.2 | `rpc: complete_profile(display_name, avatar_url, degree_level, department?, gender?, bio?, contact_ig?, contact_line?, contact_discord?, default_campus?)` | 註冊硬性門檻（SPEC §2, v1.4；`bio`/頭像規則 v1.33）：`avatar_url` 必填（且不可為 `dicebear.com` 自動產生的佔位頭像，v1.33）+ `bio` 必填（v1.33 起，不再選填）+ `degree_level` (enum `UNDERGRAD\|MASTER\|PHD`) 必選 + 三項聯絡方式至少一項。`department` 仍選填、不卡門檻。未完成必填項前所有 Request 類 RPC 回 `PROFILE_INCOMPLETE`（`bio`/頭像佔位分別對應 `detail: BIO_REQUIRED`/`PLACEHOLDER_AVATAR_NOT_ALLOWED`）。**`school` 不是參數**——由 email 網域 mapping（`nycu.edu.tw → NYCU`、`nthu.edu.tw → NTHU`）在 server 端寫入，DB CHECK 保證與網域一致（SPEC §2：不讓使用者自選）。`default_campus`（v1.32，選填）：註冊畫面在校區選項 ≥2 個時要求選一次；`on conflict do update` 這條路徑同時服務「編輯個人資料」重複呼叫（不帶這個參數），用 `coalesce` 確保沒帶值不會洗掉先前存好的校區。`bio`/頭像的兩條新檢查同樣套用到這條編輯路徑，見 SPEC v1.33。 |
| 1.3 | `PATCH app_user`（PostgREST，RLS：own row） | 更新個人資料（含 `bio`, `department`）；DB CHECK 保證改完仍滿足硬性門檻與必填約束。也用於寫入 `onboarding_seen_at`（v1.20，新手上手引導卡片已讀時間戳，見 `docs/UI_PLAN.md` §11.1）與 `default_campus`（v1.32，`create_request` 送出時若選了跟目前預設不同的校區即回寫，等同「最近使用的校區」自動成為下次預設），不另開 RPC。 |
...
| # | Endpoint | 說明 |
|---|---|---|
| 2.1 | `GET activity_type?status=eq.APPROVED`（PostgREST） | 公開類型清單。回傳包含 `default_min_participants`, `default_max_participants`, `group_size_step`（v1.5 / v1.6）、`description`（v1.10，前端「?」按鈕顯示的玩法說明，nullable）、`skill_level_enabled`（v1.34，boolean，預設 `false`）。<br>🟢 **選單計算規範（ERD 備註 22）**：前端建立 Request 選項卡時，若 `group_size_step` 非 null（如 `step=1` 或 `step=2`），從 min 到 max 按 step 算出的數值為唯一可選的離散人數選項；`group_size_step` 為 null 時才渲染為連續區間選單。<br>🟢 **Skill Level 顯示規範（v1.34，ERD 備註 48）**：前端只有在 `skill_level_enabled=true` 時才顯示程度選擇器（見 3.1）；admin 可在 Studio 審核任何新類型（含使用者提案通過的）時直接打開這個 flag，不需要改任何程式碼。 |
| 2.2 | `rpc: search_activity_type(query)` | 新增前的模糊比對/autocomplete（SPEC §5，防「羽球 vs 羽毛球」重複稀釋配對池）。🟡 **v1.38**：回傳排序改為 `order by sort_order, name`（原本只有 `name`，即中文字面排序）——`activity_type.sort_order` 是 admin 可在 Studio 直接調的 per-type 欄位，預設 100，目前運動類 10 / 讀書 20 / 麻將 900。前端直接照回傳順序渲染，不在 client 端重排。 |
...
|---|---|---|---|
| 3.1 | `rpc: create_request(activity_type_id, campus, earliest_start, latest_start, min_participants, max_participants?, allow_downgrade, sport_level?, sport_level_rating?, study_target?)` | R1 | 🔴 **（v1.16）**`bucket` 參數移除，改由呼叫端直接傳入換算好的 `earliest_start`/`latest_start`（timestamptz）；時段桶（NOW/TODAY/TONIGHT/TOMORROW_AM 等）是 UI 層的呈現方式，換算邏輯移回前端，backend 只驗證範圍合法性（SPEC §4、v1.16 變更紀錄）：① `latest_start <= earliest_start` → `INVALID_INPUT` detail `LATEST_START_MUST_BE_AFTER_EARLIEST_START`；② `latest_start > now() + 24h` → `WINDOW_EXCEEDS_24H`；③ `latest_start < now()` → `INVALID_INPUT` detail `LATEST_START_IN_PAST`；`earliest_start` 不設下限（Matching Engine 用 `greatest()` 取實際 `start_time`，過早不影響正確性）。`min_participants >= 2`；`max_participants` 為 upper bound (null 則 fallback 至 `activity_type.default_max_participants`)。<br>🟢 **驗證規範**：① `min/max_participants` 計數**包含 owner 本人**。② 若 `group_size_step` 非 null，帶入的人數必須符合離散步階選項，否則回傳 `INVALID_GROUP_SIZE_OPTION`。③ **（v1.11）** `campus` 不再是精確地點 FK，而是 Matching Scope 的自由文字校區值；檢查 owner 的 `school` 底下是否存在至少一筆 `status='APPROVED'` 且 `campus` 相符的地點，否則回 `INVALID_CAMPUS_SCOPE`（SPEC §6/§7 同校隔離，取代 v1.10 及之前版本的 `SCHOOL_LOCATION_MISMATCH`；`school` 本身不對——目前 create_request 已不接受 `school` 參數，不會發生——才會是 `SCHOOL_LOCATION_MISMATCH`，見 3.8）。④ 僅建立 owner 本人的 Request（此時 `request_member` 只有 owner 一人）；邀請朋友加入一律透過 3.7 生成邀請連結，好友透過 3.8 加入，在 Matching Engine 掃描撮合前陸續完成組隊。精確地點（Activity Location）改成配對成立後才由參與者投票決定，見 6.4/6.5。<br>🟢 **`sport_level` / `sport_level_rating`（v1.42 重構取代舊 generic skill_level）**：`sport_level`（選填 text，運動等級代碼，例如 HIGH、LEVEL_8_10、NTRP_3_5、REGULAR_PLAYER）只在該 `activity_type.level_system != 'NONE'`（或 `skill_level_enabled=true`）時保留，否則強制為 `null`；`sport_level_rating`（選填 int，例如桌球積分 1450）只在支援積分的運動（如桌球）保留。撮合相容性由 `fn_sport_level_match` 依各運動分級規則進行相鄰容許判定，`null` 視為 wildcard。<br>🟢 **`study_target`（v1.35，選填，text，讀書同伴目標）**：只在該 activity_type 名稱為「讀書」時才有意義，否則實際寫入一律強制為 `null`（理由同上）。傳入原始文字，backend 同時計算並存入正規化比對值（`fn_normalize_study_target`：去頭尾空白、全形括號/空白轉半形、統一小寫），撮合時只比對正規化值，`GET`/其他讀取端一律拿到未經清理的原文。 |
...
| 6.1.1 | `rpc: get_activity_member_profiles(activity_id)`（v1.23，`bio` 為 v1.33 新增，`level_system`/`sport_level`/`sport_level_rating`/`study_target` 為 v1.34/v1.35/v1.42 新增） | — | **成員名單的個人資料補充出口**：`app_user` 的 `own_profile_select` RLS 只允許查自己，6.1 拿不到其他成員的 `school`/`department`/`degree_level`/`bio`/可信度等級，這支 RPC 補上。呼叫者須為該活動 `JOINED` 成員（同 6.2 判準），否則 `NOT_ACTIVITY_MEMBER`。回傳該活動全體成員（不論 `JOINED`/`CANCELLED`，同 6.2 不過濾）的 `user_id`/`school`/`department`/`degree_level`/`bio`/`reliability_tier`（複用 `fn_reliability_tier`）/`level_system`/`sport_level`/`sport_level_rating`/`study_target`。刻意不含 `display_name`/`avatar_url`/`contacts`——6.2 已無條件回傳前兩者，避免兩支 RPC 重複欄位；前端依 `user_id` 合併兩支 RPC 的結果。這些欄位屬一般個人資料，不比照 `contacts` 加時效/再約閘門。`bio`（v1.33）供前端「個人檔案卡」使用——點開成員頭像顯示照片＋自我介紹＋科系等，見 SPEC v1.33 變更紀錄。`sport_level`/`sport_level_rating`/`level_system`（v1.42）與 `study_target` 是該成員原始 Request（透過 `activity_member.source_request_id` join `match_request` join `activity_type`）的運動等級/積分/讀書目標；`study_target` 回傳的是原文欄位，不是正規化比對值（見 3.1）。 |
...
| 6.4 | `rpc: propose_activity_location(activity_id, location_id, custom_name)`（v1.11，`custom_name` 為 v1.30 新增） | — | **提案 Activity Location 候選（SPEC §9.1）**：呼叫者須為該活動成員，且活動 `status in (MATCHED, ONGOING)`（v1.37 起不再有「鎖定後整組擋掉」這件事，否則回 `ACTIVITY_NOT_ACTIVE`，跟 6.6/6.7 同一個碼）；`location_id`/`custom_name` 恰好給一個，都給或都不給回 `INVALID_INPUT`。給 `location_id` 時須屬於該活動的 `(school, campus)` 範圍內、`status='APPROVED'` 的地點（否則 `INVALID_CAMPUS_SCOPE`），跟 v1.11 行為相同。給 `custom_name`（v1.30，1~40 字，超出回 `INVALID_INPUT`）時**不做任何審核或範圍檢查**，直接建立僅該活動可見的候選，且不寫入 `location` 表——見 SPEC v1.30 變更紀錄。提案動作同時視為投給該候選一票；若該候選已存在（他人先提過同一 `location_id`，或同名——大小寫不敏感——的 `custom_name`），本次呼叫退化成投票，不視為錯誤。成功後即時重新計票並覆寫 `activity.activity_location_id`（v1.37，見 6.5 備註）。 |
| 6.5 | `rpc: vote_activity_location(activity_id, option_id)`（v1.11，v1.30 起參數由 `location_id` 改名 `option_id`） | — | **對既有候選投票（SPEC §9.1）**：呼叫者須為該活動成員，且活動 `status in (MATCHED, ONGOING)`（同 6.4，v1.37 起不再有鎖定閘門）；`option_id` 須是該活動既有的 `activity_location_option.id`（先前透過 6.4 建立），否則回 `NOT_FOUND`。一人一票，可改票（重複呼叫覆寫先前的投票）。🟡 **v1.37**：每次提案/投票成功後都會即時依目前得票數重新計算並覆寫 `activity.activity_location_id`（v1.30 起是 `activity_location_option.id`，不是 `location.id`——見下方 §9.1 備註）——不再是「`start_time` 那一刻凍結寫死一次」的欄位，`fn_start_activities()` 只在轉 `ONGOING` 前做一次同樣計算當保底防禦；同票一律取最早提案者勝出（見 SPEC §9.1、ERD 設計備註 31/32/46）。 |
...
錯誤碼：`NOT_ACTIVITY_MEMBER`、`INVALID_CAMPUS_SCOPE`（v1.11，6.4 給 `location_id` 時該地點不屬於該活動的 `(school, campus)` 範圍）、`NOT_FOUND`（v1.11，6.5 投給一個尚不存在的候選 `option_id`）、`ACTIVITY_NOT_ACTIVE`（v1.11.1 起用於 6.6/6.7；v1.14.1 起 6.3 也重用同一個碼；v1.24 起 6.8、v1.28 起 6.9 亦重用；**v1.37 起 6.4/6.5 也重用同一個碼，取代已刪除的 `ACTIVITY_LOCATION_LOCKED`**，見上方各節說明）、`MEETING_POINT_UPDATE_COOLDOWN`（v1.11.1，6.6 冷卻中）、`INVALID_INPUT`（v1.11.1，6.6 空白描述 / 6.7 提示超過 30 字；v1.28，6.9 標籤數量/長度超限；v1.30，6.4 `location_id`/`custom_name` 沒有恰好給一個，或 `custom_name` 長度超過 40 字）

🔴 **v1.37 起 `ACTIVITY_LOCATION_LOCKED` 已從系統移除**（曾用於 v1.11～v1.36，6.4/6.5 在活動已鎖定候選或 `status <> MATCHED` 時拋出）：真實使用者回報零候選、活動已過 `start_time` 轉 `ONGOING` 的情境下，畫面仍顯示提案/投票按鈕，一按卻整組被這個碼擋下，體驗上等同「叫你做一件系統自己拒絕的事」。現在 6.4/6.5 的閘門條件跟 6.6/6.7 完全一致（`status in (MATCHED, ONGOING)` 即可，`COMPLETED`/`CANCELLED` 才擋，回 `ACTIVITY_NOT_ACTIVE`），`activity_location_id` 也不再是「鎖定」語意，而是持續即時計票的欄位，見 SPEC v1.37 變更紀錄、§9.1。
...
| Activity 開始 | 每分鐘 | A2 | 🟢 **v1.11 起第一次真正落地成 SQL**（`fn_start_activities()`；先前只有本表格描述，沒有對應函式，見 `app/lib/rpc/RPC_COVERAGE.md`）：`start_time` 已到 → 依目前得票數重新計算 Activity Location（同票取最早提案者，零候選則維持 `NULL`，見 SPEC §9.1）→ `ONGOING`；發送 `ACTIVITY_REMINDER` 通知。🟡 **v1.37**：這裡的計票只是保底防禦，`activity_location_id` 已由 6.4/6.5 持續即時維護，不再是這一步才第一次寫入。 |
...
| §2 `department` 選填、不進配對邏輯 (v1.4)；`bio` 註冊硬性門檻 (v1.3 新增／v1.33 改硬性門檻)，同樣不進配對邏輯 | `department`：1.2/1.3（無門檻檢查）+ ERD 無 NOT NULL；`bio`：1.2 `BIO_REQUIRED` 檢查 + ERD NOT NULL DEFAULT '' |
...
| 13.2 | `rpc: get_campus_demands(school, campus)` | **匿名活動需求卡聚合查詢（v1.44）**：回傳該 `(school, campus)` 底下目前 `REQUESTING` 中且尚未過期（`latest_start > now()`）的有效活動需求卡清單。依據活動條件與人數範圍分組聚合（`activity_type_id`, `activity_type_name`, `campus`, `earliest_start`, `latest_start`, `sport_level`, `sport_level_rating`, `study_target`, `min_participants`, `max_participants`），回傳包含 `person_count`（實際等待中人頭數，不含 LEFT）與 `request_count`（組數）。時間篩選（`全部`/`現在`/`今天`/`明天`）由前端依時間區間重疊（含跨日支援）進行過濾。嚴格遵循匿名與盲配邊界，不回傳任何 user ID、request ID、姓名、頭像或個資。前端每 30 秒輪詢更新。 |
