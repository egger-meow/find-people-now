...
begin
  ...
  insert into location (school, campus, name, is_active) values ('NYCU', v_campus, '第二餐廳前', true)
  ...
  insert into match_request (owner_id, activity_type_id, school, campus, earliest_start, latest_start, min_participants, max_participants, status)
  values (v_owner, v_act_type_id, 'NYCU', v_campus, now(), now() + interval '2 hours', 2, 4, 'MATCHED')
  ...
  update fixtures set
    ...
    act_type_id = v_act_type_id, campus = v_campus, req1_id = v_req1.id, req2_id = v_req2.id;
end;
$setup$;
