...
-- -----------------------------------------------------------------------------
-- 2. 運動類（籃球/羽球/跑步/健身/網球/桌球）全部是 10
-- -----------------------------------------------------------------------------

select is(
  (select count(*)::int from activity_type
    where name in ('籃球', '羽球', '跑步', '健身', '網球', '桌球') and sort_order = 10),
  6,
  'v1.38/v1.42：六個運動類型的 sort_order 都是 10'
);
...
-- 斷言的是完整順序，不只是欄位值。同 sort_order 內部 fallback 到 name，
...
-- -----------------------------------------------------------------------------
-- 「球」同時命中桌球（10）、籃球（10）、網球（10）與羽球（10）——同 sort_order，
-- 驗證 fallback 到 name 之後順序仍然穩定，不會因為加了 where 條件就回到不定序。

select is(
  (select array_agg(name order by ord)
     from (select name, row_number() over () as ord from search_activity_type('球')) s),
  array['桌球', '籃球', '網球', '羽球'],
  'v1.38/v1.42：模糊查詢結果一樣套用 sort_order/name 排序'
);
